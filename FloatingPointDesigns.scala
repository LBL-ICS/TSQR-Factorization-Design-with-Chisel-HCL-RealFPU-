...
//      println(s"addper layer: ${add_per_layer}")
  ...
  class hqr5(bw: Int) extends Module{
    ...
    val adder = Module(new FP_adder_13ccs(bw)).io
    val subtractor = Module(new FP_subtractor_13ccs(bw)).io
    adder.in_en := true.B
    adder.in_a := io.in_a
    adder.in_b := io.in_b
    subtractor.in_en := true.B
    subtractor.in_a := io.in_a
    subtractor.in_b := io.in_b
  ...
  class hqr7(bw: Int) extends Module{
    ...
    val multiplier = Module(new FP_multiplier_10ccs(bw)).io
    val reciprocal = Module(new FP_reciprocal_newfpu(bw,1)).io
    multiplier.in_en := true.B
    reciprocal.in_en := true.B
    reciprocal.in_a := io.in_a
...
//      }
//    }
    val sw5 = new PrintWriter("hqr7_chisel.v")
    sw5.println(getVerilogString(new hqr7(32)))
    sw5.close()
  }
