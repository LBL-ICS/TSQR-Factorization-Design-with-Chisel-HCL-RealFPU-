...
object FPComplex { // these are the complex FP modules
  ...
  class FPComplexAdder_v2(bw: Int) extends Module {
    ...
    val FP_adders = (for (i <- 0 until 2) yield {
      val fpadd = Module(new FP_adder_13ccs(bw)).io
      fpadd
    }).toVector
...
// change fp operators
  ...
  class FPComplexMult_v2(bw: Int) extends Module {
    ...
    val FP_sub = Module(new FP_subtractor_13ccs(bw)).io
    val FP_add = Module(new FP_adder_13ccs(bw)).io
    val FP_multipliers = (for (i <- 0 until 4) yield {
      val fpmult = Module(new FP_multiplier_10ccs(bw)).io
      fpmult
    }).toVector
    for(i <- 0 until 4){
  ...
  class FPReg(depth: Int,bw: Int) extends Module{
   ...
   // val reg = RegInit(VecInit.fill(depth)(0.U(bw.W)))
    ...
    val reg = RegInit(VecInit.fill(depth)(0.U.asTypeOf(new ComplexNum(bw))))
      reg(0) := io.in
      for(i <- 1 until depth){
        reg(i) := reg(i-1)
      }
  ...
  class FP_DDOT_dp_complex(bw: Int, n: Int) extends Module {
    val io = IO(new Bundle{
      val in_a = Input(Vec(n, new ComplexNum(bw)))
      val in_b = Input(Vec(n, new ComplexNum(bw)))
      val out_s = Output(new ComplexNum(bw))
    })
    var temp_n = n
    ...
    val multipliers = Vector.fill(n)(Module(new FPComplexMult_v2(bw)).io)
    ...
    if(add_per_layer.nonEmpty) {
      val regs_and_adds = for (i <- 0 until add_per_layer.length) yield {
        val adds = for (j <- 0 until add_per_layer(i)) yield {
          Module(new FPComplexAdder_v2(bw)).io
        }
  ...
  class axpy_dp_complex(bw:Int,level:Int)extends Module {
    val io = IO {
      new Bundle() {
        val in_a = Input(new ComplexNum(bw))
        val in_b = Input(Vec((level), new ComplexNum(bw)))
        val in_c = Input(Vec((level), new ComplexNum(bw)))
        val out_s = Output(Vec((level), new ComplexNum(bw)))
      }
    }

    val multiply_layer = for(i <- 0 until level)yield{
      val multiply = Module(new FPComplexMult_v2(bw)).io
